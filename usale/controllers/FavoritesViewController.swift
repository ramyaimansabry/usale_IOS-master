...
//  usale
//
//  Created by Ahmed masoud on 7/31/18.
...
class FavoritesViewController: UIViewController, FavoriteOfferDelegate, UITableViewDelegate, UITableViewDataSource {
    var offers:[Offer] = []
    var favorites:[Int] = []
    ...
    fileprivate func loadOffers(){
        ...
        ApiManager.sharedInstance.getFavorites() { (valid, msg, offers, favorites) in
            spinner.dismiss()
            if valid{
                self.offers = offers
